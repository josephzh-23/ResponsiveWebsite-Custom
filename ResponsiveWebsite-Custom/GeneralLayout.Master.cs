...
namespace ResponsiveWebsite_Custom
{
    ...
    public partial class GeneralLayout : System.Web.UI.MasterPage
    {
        ...
        public void BindCategories()
        {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("select * from tblCategories", con))
                {
        ...
        protected void OnItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                using (MySqlConnection con = new MySqlConnection(CS))
                {
                    using (MySqlCommand cmd = new MySqlCommand(string.Format("SELECT * FROM tblSubCategories WHERE MainCatID='{0}'", catId), con))
                    {
                        cmd.CommandType = CommandType.Text;
                        using (MySqlDataAdapter sda = new MySqlDataAdapter(cmd))
                        {
